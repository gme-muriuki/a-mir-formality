...
fn append_coverage_chapters(
    ...
    // Emit a compact machine-readable summary next to the book so CI can fetch
    // it as a baseline and diff a PR's coverage against it. It goes under the
    // book source dir so mdbook copies it to the output root as
    // `coverage-summary.json`. Scrape independently of the rendered
    // `index` (which is keyed by judgment name and so collapses same-named
    // judgments) using the same `scrape_dir` the CLI uses, so the published
    // baseline and a PR build's summary are generated by an identical path and
    // the CI diff sees no phantom churn.
    let scraped = scrape_dir(src_dir)?;
    summary::write(
        &root
            .join(&ctx.config.book.src)
            .join("coverage-summary.json"),
        &summary::build(&scraped, &cov),
    )?;
